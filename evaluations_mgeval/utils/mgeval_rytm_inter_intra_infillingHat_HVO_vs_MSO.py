...
if __name__ == '__main__':
    ...
    gt_features = get_feature_dict_from (gt_hvo_seqs_downsampled, set_tag="Ground Truth", use_voices=use_voices)
    ...
    feature_dict = deepcopy(gt_features)
...
'''
    ...
    allowed_analysis = ['Statistical::Total Step Density',
    ...
    for set_labels in eval_labels:
        ...
        if generate_csv is True:
            ...
            csv_path = os.path.join(fig_path, "temp.csv")
        ...
        if generate_these is True:
            fig_, axes_ = plot_inter_intra_pdfs(
                raw_data, fig_path, set_labels, show=True, figsize=(20, 4), fs=15, legend_fs=10,
                ncols=5, plot_all_intra=False, share_legend=True, bbox_to_anchor = (0.6, 0.5))
        ...
        if generate_these is True:
            fig_1, axes_1 = plot_intersets(
                ...
                legend_fs=9 , fs=11, add_legend=True, add_text=False, min_line_len_for_text=.1,legend_loc='upper right',
                set1_name=set_labels[1], set2_name=set_labels[2], legend_ncols=1, force_xlim=(0, 0.2) , force_ylim=(0.55, 1))
